...
(function()
{
    ...
    WK_Niai.prototype.populateNiaiSection = function(kanji, curPage = this.wkItemInfoRef.currentState.on)
    {
        ...
        if (typeof wkof === `object`)
            this.update_wk_cache(similar_list); // may not be needed anymore with changes to NiaiDB
        ...
        if (curPage !== `itemPage`)
            $(`.subject-character`).attr(`data-turbo-frame`, `_blank`);
        else
            $(`.subject-character:not(li.notInWK a)`).attr(`data-turbo-frame`, `_blank`);

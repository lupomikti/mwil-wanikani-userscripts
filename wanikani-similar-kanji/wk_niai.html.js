...
(function () {
    ...
    WK_Niai.prototype.gen_item_chargrid =
        ({ kanji, readings, meanings, wk_level, is_locked = ``, notInWK = ``, badge = `item-badge`, href = `javascript:;`, kanji_id = `` }) =>
            `<li id="${kanji_id}" class="subject-character-grid__item ${notInWK}" data-kanji="${kanji}">
                <span lang="ja" class="${badge}" data-kanji="${kanji}"></span>
                <a class="subject-character subject-character--kanji subject-character--grid ${is_locked === '' ? is_locked : `subject-character--${is_locked}`}" title="${kanji} | ${notInWK !== ''  ? `Not in WK` : `WK Level: ${wk_level}`}" href="${href}">
                    <div class="subject-character__content">
                        <span class="subject-character__characters" lang="ja">${kanji}</span>
                        <div class="subject-character__info">
                            <span class="subject-character__reading">${readings[0]}</span>
                            <span class="subject-character__meaning">${meanings[0]}</span>
                        </div>
                    </div>
                </a>
            </li>`;
...
(function () {
    ...
    WK_Niai.prototype.createNiaiSection = function (style) {
        const $section = $(`<section></section>`)
            ...
            .attr(`style`, style)
            .addClass(`subject-section ${GM_info.script.namespace} col1`);

        const $main_fold = $(`<section></section>`)
            .attr(`id`, `niai_main_fold`)
            .addClass(`subject-section__content`);

        const $grid_container = $(`<div></div>`)
            .addClass(`subject-character-grid`);

        const $grid = $(`<ol></ol>`)
            .attr(`id`, `niai_similar_grid`)
            .addClass(`subject-character-grid__items`);
        ...
        $main_fold.append($grid_container);
    ...
    WK_Niai.prototype.onSimilarInput = function (event) {
        ...
        if (!new_kanji || new_kanji.length > 1) {
            $(`#kanji-dropdown .subject-character__characters`).text(`？`);
            $(`#kanji-dropdown .subject-character__reading`).html(`&nbsp;`);
            $(`#kanji-dropdown .subject-character__meaning`).text(`Enter Kanji`);
            $(`#kanji-dropdown`)[0].dataset.kanji = ``;
        }
        else if (!this.ndb.isKanjiInDB(new_kanji)) {
            $(`#kanji-dropdown .subject-character__characters`).text(new_kanji);
            $(`#kanji-dropdown .subject-character__reading`).html(`&nbsp;`);
            $(`#kanji-dropdown .subject-character__meaning`).text(`Not In WK!`);
            $(`#kanji-dropdown`)[0].dataset.kanji = ``;
        }
        else {
            ...
            $(`#kanji-dropdown .subject-character__characters`).text(new_kanji);
            $(`#kanji-dropdown .subject-character__reading`).text(kanji_item.readings[0]);
            $(`#kanji-dropdown .subject-character__meaning`).text(kanji_item.meanings[0]);

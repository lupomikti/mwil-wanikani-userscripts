   ...
   "use strict";
    ...
    NiaiDB.prototype = {
        ...
        isKanjiInWK: function(kanji)
        {
            ...
            if (!this.isKanjiInDB(kanji) && !(kanji in this.wkof_items)) // if the kanji is not in our local db and is not in the list fetched from wkof then it is not in wk
                return false;
            else if (!this.isKanjiInDB(kanji) && (kanji in this.wkof_items)) // if the kanji is not in our local db but is in the list fetched from wkof then it is in wk
                return true;

            return (this.lookup_db[kanji].level !== 99); // if the kanji is in our local db, we don't care about the wkof list; it is in wk if the level is not 99
        },
        ...
        getSimilar: function(kanji, level, sources, min_score)
        {
            ...
            result.sort((a,b) => b.locked - a.locked
    ...
    WK_Niai.prototype.update_wk_cache = function(similar_list)
    {
        ...
        similar_list.forEach((sim_kanji) => {
            ...
            if (item) {
                const is_locked = !item.assignments?.unlocked_at;
                $(`li[id$="${sim_kanji}"] a`)
                ...
                $(`li[id$="${sim_kanji}"] a.subject-character`)
                    .attr(`title`, `${sim_kanji} | WK Level: ${item.data.level}`);
            }
        });
    };
    // #########################################################################
